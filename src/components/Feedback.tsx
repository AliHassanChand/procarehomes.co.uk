...
export default function Feedback({ onNavigate }: FeedbackProps) {
  ...
  const genericFeedbacks = [
    ...
    },
    {
      ...
      score: 5,
      text: "PRO Care Homes' clinicians provide excellent compliance backups. Their live Nourish planning accounts for every trigger, and the team walks beside the resident rather than stepping back during difficult events."
    },
    {
      name: "Kirsty W.",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white border rounded-2xl p-6 shadow-xs space-y-1 hover:border-sky-300 transition duration-300">
              ...
              <span className="text-[11px] uppercase font-bold tracking-wider text-slate-500 font-mono block">{stat.label}</span>
            </div>
          ))}
        </div>
